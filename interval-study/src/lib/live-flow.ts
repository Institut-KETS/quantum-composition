...
export type FlowTrail = {
    notes: WrittenNote[];
    previous?: WrittenNote;
    opacity: number;
};
...
export function drawLiveFlow(ctx: CanvasRenderingContext2D, width: number, notes: WrittenNote[], startBeat: number, selected: number | null, terminalIndex: number | null, previous?: WrittenNote, history: FlowTrail[] = []) {
    ...
    for (let beat = startBeat; beat <= end; beat++) {
        ctx.strokeStyle = beat % 4 === 0 ? '#d6ded3' : '#edf1ea';
        ctx.lineWidth = beat % 4 === 0 ? .8 : .5;
        ctx.beginPath();
        ctx.moveTo(x(beat), 7);
        ctx.lineTo(x(beat), 117);
        ctx.stroke();
    }
    ...
    PITCHES.forEach((pitch, row) => {
        ...
        ctx.beginPath();
        ctx.moveTo(66, y(row));
        ctx.lineTo(x(end), y(row));
        ctx.stroke();
        ctx.fillStyle = '#5b6860';
        ctx.fillText(pitch.label, 5, y(row));
    });
    const selectedNote = notes.find(note => note.index === selected);
    if (selectedNote) {
        ctx.fillStyle = '#dce8db88';
        ctx.fillRect(x(selectedNote.onset), 7, x(selectedNote.onset + selectedNote.beats) - x(selectedNote.onset), 110);
    }
    const path = (record: WrittenNote[], preceding?: WrittenNote) => {
        ...
        if (preceding) {
            ...
            ctx.moveTo(Math.max(left, x(first.onset)), y(first.pitch));
        record.forEach((note, index) => {
            const next = record[index + 1];
            if (next)
                transition(next.onset, note.pitch, next.pitch);
            else
                ctx.lineTo(x(note.onset + note.beats), y(note.pitch));
        });
    };
    // Oldest first: fading paths are actual earlier takes only, never an ensemble
    // of unplayed possibilities.
    ctx.strokeStyle = '#658372';
    ctx.lineWidth = 1.3;
    ctx.lineCap = 'butt';
    ctx.lineJoin = 'miter';
    for (const take of history) {
        ctx.globalAlpha = take.opacity;
        ctx.beginPath();
        path(take.notes, take.previous);
        ctx.stroke();
    }
    ctx.globalAlpha = 1;
    if (notes.length) {
        ctx.strokeStyle = '#176345';
        ctx.lineWidth = 1.85;
        ctx.lineCap = 'butt';
        ctx.lineJoin = 'miter';
        ctx.beginPath();
        ...
        if (terminal) {
            ctx.beginPath();
            ctx.arc(x(terminal.onset + terminal.beats), y(terminal.pitch), 3.2, 0, 2 * Math.PI);
            ctx.fillStyle = '#176345';
    ...
    ctx.fillText(notes.length ? tr("Measured pitch path; horizontal length is the selected interval.") : tr("Earlier played takes fade here after a fresh take begins."), 5, 129);
}
