...
type Laws = Record<LawKind, number[]> & {
    kernelResidual: number;
};
type Observed = {
    pitch: number;
    kind: LawKind;
...
function LawCell({ kind, value, selected }: {
    kind: LawKind;
    value: number;
    selected: boolean;
}) {
...
export const FigureLaws = memo(function FigureLaws({ laws, passage, entrance, observed }: {
    laws: Laws;
    ...
    return <aside className="figure-laws" aria-label={tr("Live readout laws for the selected passage")}>
    <div className="figure-panel-heading"><span className="figure-letter">(c)</span><span>{tr("Step and endpoint laws")}</span><span className="law-live-tag">{tr("live")}</span></div>
    <p className="law-context"><span>|{BASIS[entrance]}⟩</span><span>{blocks.first} | {blocks.second}</span></p>
    <div className="law-grid" role="table" aria-label={tr("Conditional readout laws for {0} followed by {1} from {2}", blocks.first, blocks.second, BASIS[entrance])}>
      <span className="law-outcome-heading" role="columnheader">{tr("outcome")}</span>
      <span className="law-column-heading law-middle-heading" role="columnheader"><strong>{tr("Step Z")}</strong><small>p<sub>Z</sub>(z) = Pr(Z = z | x)</small></span>
      <span className="law-column-heading law-coherent-heading" role="columnheader"><strong>{tr("Endpoint Y")}</strong><small>p<sub>0</sub>(y) = Pr(Y = y | S = 0, x)</small></span>
      <span className="law-column-heading law-checked-heading" role="columnheader"><strong>{tr("Endpoint Y")}</strong><small>p<sub>1</sub>(y) = Pr(Y = y | S = 1, x)</small></span>
    ...
    <p className="law-note">{tr("The third column marginalizes the step outcome Z. The kernel identity is distinct from the displayed |")}{BASIS[entrance]}{tr("\u27E9 endpoint vector.")}</p>
  </aside>;
});
