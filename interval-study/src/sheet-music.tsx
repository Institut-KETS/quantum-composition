...
export const SheetMusic = memo(function SheetMusic({ notes, selected, playing, onSelect, showFlow, totalCount, history = [], fadeProgress = 0 }: {
    notes: WrittenNote[];
    ...
    useEffect(() => {
        ...
        if (system && (system.offsetTop < viewport.scrollTop || system.offsetTop + system.offsetHeight > viewport.scrollTop + viewport.clientHeight))
            viewport.scrollTop = system.offsetTop;
    }, [selected, playing, notes.length]);
    return <div ref={container} className="sheet-music live-score-window" tabIndex={0} aria-label={tr("Live treble-clef score on a sixteen-beat ruler. Only notes that have sounded are shown.")}>
    {rows.length === 0 ? <div className="empty-score">
      <div className="staff"><div className="staff-lines" aria-hidden="true"/><span className="treble-clef" aria-hidden="true">𝄞</span><div className="beat-ruler" aria-hidden="true">{Array.from({ length: SCORE_BEATS_PER_SYSTEM + 1 }, (_, beat) => <span className={beat % 4 === 0 ? 'major-beat' : ''} style={{ left: liveBeat(beat, 0).left }} key={beat}>{beat % 4 === 0 ? beat : ''}</span>)}</div><p>{tr("Press Play piano. Each note will be written at its measured beat.")}</p></div>
      {showFlow && <p className="empty-flow">{tr("The sampled path will grow beneath the score. No future outcomes are drawn.")}</p>}
    </div> : rows.map((row, rowIndex) => <div className="score-system" key={row.startBeat} aria-label={tr("Score beats {0} to {1}", row.startBeat, row.startBeat + SCORE_BEATS_PER_SYSTEM)}><div className="staff">
      <div className="staff-lines" aria-hidden="true"/>
      <span className="treble-clef" aria-hidden="true">𝄞</span>
      <span className="staff-counter">{tr("beat") + " "}{row.startBeat}</span>
